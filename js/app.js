...
var taskApp = angular.module('TaskApp', ['ngAnimate', 'ngDragDrop', 'ngCookies'], function($httpProvider) {
...
taskApp.config(function($sceProvider) {
...
taskApp.directive('dynamic', function ($compile) {
  return {
    restrict: 'A',
    replace: true,
...
taskApp.directive('ngEnter', function($document) {
    return {
...
taskApp.directive('ngPassword', function($document) {
    return {
      link: function(scope, element, attrs) {
        var enterWatcher = function(event) {
          if (event.which === 13) {
